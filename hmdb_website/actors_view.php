    ...
    <h5 style="text-align: center">The Habesha Movie Database</h5>
        ...
        <nav id="mainNav">
            <ul class="centered">
                ...
                <li><a href="#" class="thispage">Actors</a></li>
  ...
  //          echo "<p>-------------------------- $tmp   ---  $tmp[0] ---- $tmp[1]</p>";
            ...
            if(isset($_GET["person_info"])){
                ...
                if($person_info_id != "false"){
                    ...
                    $cur_person_awards = "SELECT award.award_name, award.agency, award_given.date_given FROM award, award_given WHERE award.award_id = award_given.award_id AND award_given.person_id = $person_info_id";
                    ...
                    for($o = 0; $o < $n_award_info; $o++){
                        ...
                        $info_award_given_date[$o] = mysql_result($person_info_awards, $o, 'date_given');
                    }
                    ...
                    $info_quote = [];
                    $n_quotes_info = mysql_num_rows($person_info_awards);
                    for($o = 0; $o < $n_quotes_info; $o++){
                        ...
                        $info_quote_movie_release_date[$o] = mysql_result($person_info_quote, $o, 'released_date');
                    }
			  		...
			  		<div style="margin-top: 151px;" class="popup-content">
			    		<div class="modal-top">
			      			<b><?php 
                                    if(!$editing_actor)
                                        ...
                                        echo "<form method=\"post\" action=\"actors_view.php?person_info=$person_info_id\">";
						...
						<div class="popup-content-install2">
			  				...
			  				<div class="modal-text-cont">
                                ...
                                <?php
                                    if(isset($_COOKIE["admin"]) && !$editing_actor){//allowing the admin to edit or delete a movie
                                        echo "<a href=\"actors_view.php?delete_person=$person_info_id\" style=\"color: red;\">Delete</a>";
                                        echo " | ";
                                        echo "<a href=\"actors_view.php?person_info=$person_info_id&edit_person=$person_info_id\" style=\"color: blue;\">Edit</a>";
                                    }
                                ?>
                                ...
                                <p style="font-size:14px;font-weight:normal;margin:0 0 30px;line-height:30px;"></p>
                                    ...
                                    <div class="btn-dwn-cont">
				  					...
				  					<span><span class="browser-icon browser-icon-big" style="background-image:url('/img/browser/firefox.png')"></span></span>
                                            ...
                                            <a href="actors_view.php" style="font-size: 14px;"><div class="close-modal close-modal-icon"></div></a>
...
//                $mv_title = sanitize($tmp[1]);
                ...
                echo "<p>////////////////////////  $fname</p>";
                $actors = "SELECT * FROM people WHERE first_name like '%$fname%' or last_name like '%$fname%' AND person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY person_id LIMIT 25";
            }
            else if(isset($_GET["country"])){
                //order by country
                $actors = "SELECT * FROM people WHERE person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY country LIMIT 25";
            }
            else if(isset ($_GET["first"])){
                //order by title
                $actors = "SELECT * FROM people WHERE person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY first_name LIMIT 25";
            }
            else if(isset($_GET["last"])){
                //order by released date
                $actors = "SELECT * FROM people WHERE person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY last_name DESC LIMIT 25";
            }
            else if(isset ($_GET["birthday"])){
                //order by rating
                $actors = "SELECT * FROM people WHERE person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY Date_of_birth DESC LIMIT 25";
            }
            else{
                //just all the movies -- order by film_id
                $actors = "SELECT * FROM people  WHERE person_id IN (SELECT person_id FROM works_on WHERE role_id = 8) ORDER BY person_id LIMIT 25";
            }

            
            
            $ac_results = mysql_query($actors);
            $num_actors = mysql_num_rows($ac_results);
            for($i = 0; $i < $num_actors; $i++){
                /////////////////////////////////
                $actor_ids[$i] = $p_id = mysql_result($ac_results, $i, 'person_id');
                $movies_worked_on = "SELECT title FROM movie WHERE movie.film_id IN (SELECT works_on.film_id FROM works_on WHERE works_on.person_id = '$p_id' AND works_on.role_id = 8)";
            ...
            for($i = 0; $i < $num_actors; $i++){//params used here are $num_movies and all the arrays
        ?>
       
        <div class="pic-thumb" >
        <img src="<?php echo $photo[$i]; ?>" style="display: inline;"> 
        <a href="actors_view.php?person_info=<?php echo $actor_ids[$i]; ?>" >
            <p id="tit"><?php echo $actor_names[$i]; ?></p>
        <br>
        <span><?php 
		...
		<li>
                    <form action="actors_view.php" method="get">
                        <input type="search" autocomplete="" name="f_name"><br><br>
                    </form>
                    <a href="actors_view.php" data-category="all" id="active-category">Order By Category 
		</a>
		</li>
		<li>
                    <a href="actors_view.php?first=true" data-category="firstname">First Name
		</a>
		</li>
		<li>
                    <a href="actors_view.php?last=true" data-category="lastname">Last Name
		</a>
		</li>
		<li>
                    <a href="actors_view.php?birthday=true"  data-category="birthd">Birthday
		</a>
		</li>
		<li>
                    <a href="actors_view.php?country=true" data-category="countr">Country
		</a>
		</li>
		</ul>
